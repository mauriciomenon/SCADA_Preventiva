# Baseline_NMR5 10.2.4 para PIC.EE.0246 - VERSAO FINAL CORRIGIDA
# Autor: Mauricio Menon
# Versão inicial: FAT NMR5 Houston (2018)
# Versão atual 08/07/2025 - VERSAO FINAL COM MELHORIAS
...
# CORRECOES: Sintaxe scriptblocks + Variavel event + Mais arquivos por metodo
...
Baseline_NMR5 10.2.4 para PIC.EE.0246
Autor: Mauricio Menon
Versão inicial: FAT NMR5 Houston (2018)
Versão atual 08/07/2025 - VERSAO FINAL
"@
...
# FUNCAO CORRIGIDA: Executar comando com todos os metodos e avaliar qualidade
function Invoke-AllMethodsWithQuality {
    ...
    # METODO 3: WMIC (Prioridade 3) - CORRIGIDO
    try {
        Write-Host "      • Tentando WMIC..." -ForegroundColor Gray
        if ($isLocal) {
            $results["WMIC"] = & $WMICCommand
        } else {
            $results["WMIC"] = & $WMICCommand -ArgumentList $Computer
        }
...
# FUNCAO CORRIGIDA: Informacoes de Hardware Completas
function Get-HardwareInformationComplete {
    ...
    # SCRIPTBLOCKS CORRIGIDOS - Adicionado ponto e virgula
    ...
    # RAM/Memory - SCRIPTBLOCKS CORRIGIDOS
    ...
    # Motherboard/ComputerSystem - SCRIPTBLOCKS CORRIGIDOS
    ...
    # Salvar melhores resultados na pasta principal + GARANTIR CSV E TXT
...
# FUNCAO CORRIGIDA: Informacoes de BIOS Completas
function Get-BIOSInformationComplete {
    ...
    # BaseBoard - SCRIPTBLOCKS CORRIGIDOS
    ...
    # Salvar melhores resultados + GARANTIR CSV E TXT
...
# FUNCAO CORRIGIDA: Analise de Servicos Completa
function Get-ServicesAnalysisComplete {
    ...
    # Services - SCRIPTBLOCKS CORRIGIDOS
    ...
    if ($servicesResults.BestResult) {
        ...
        # GARANTIR CSV E TXT para analises especificas
        if ($runningServices.Count -gt 0) {
            ...
            $runningServices | Format-Table -AutoSize | Out-File -FilePath (Join-Path $servicesPath "${Timestamp}_Servicos_Executando.txt") -Encoding UTF8 -Width 300
        }
        ...
        if ($autoServices.Count -gt 0) {
            ...
            $autoServices | Format-Table -AutoSize | Out-File -FilePath (Join-Path $servicesPath "${Timestamp}_Servicos_Automaticos.txt") -Encoding UTF8 -Width 300
        }
        ...
        if ($suspiciousServices.Count -gt 0) {
            ...
            $suspiciousServices | Format-Table -AutoSize | Out-File -FilePath (Join-Path $servicesPath "${Timestamp}_Servicos_Suspeitos.txt") -Encoding UTF8 -Width 300
        }
...
"@
    ...
    # Salvar resultado principal + GARANTIR CSV E TXT
    if ($servicesResults.BestResult) {
...
# FUNCAO CORRIGIDA: Analise de Processos Completa
function Get-ProcessAnalysisComplete {
    ...
    # Processes - SCRIPTBLOCKS CORRIGIDOS
    ...
    if ($isLocal -and $processResults.BestResult) {
        ...
        try {
            ...
            $psProcesses | Format-Table -AutoSize | Out-File -FilePath (Join-Path $processPath "${Timestamp}_Processos_GetProcess.txt") -Encoding UTF8 -Width 300
            ...
            if ($suspiciousProcesses.Count -gt 0) {
                ...
                $suspiciousProcesses | Format-Table -AutoSize | Out-File -FilePath (Join-Path $processPath "${Timestamp}_Processos_Suspeitos.txt") -Encoding UTF8 -Width 300
            }
            
            # Top processos por CPU e memoria - GARANTIR CSV E TXT
            ...
            if ($topCPU.Count -gt 0) {
                ...
                $topCPU | Format-Table -AutoSize | Out-File -FilePath (Join-Path $processPath "${Timestamp}_Top_CPU.txt") -Encoding UTF8 -Width 300
            }
            ...
            if ($topMemory.Count -gt 0) {
                ...
                $topMemory | Format-Table -AutoSize | Out-File -FilePath (Join-Path $processPath "${Timestamp}_Top_Memory.txt") -Encoding UTF8 -Width 300
            }
...
# FUNCAO CORRIGIDA: Analise de Drivers Completa
function Get-DriversAnalysisComplete {
    ...
    # System Drivers - SCRIPTBLOCKS CORRIGIDOS
    ...
    if ($isLocal) {
        try {
            ...
            if ($pnpDrivers) {
                ...
                # Drivers nao assinados - GARANTIR CSV E TXT
                ...
                if ($unsignedDrivers.Count -gt 0) {
                    ...
                    $unsignedDrivers | Format-Table -AutoSize | Out-File -FilePath (Join-Path $driversPath "${Timestamp}_Drivers_Nao_Assinados.txt") -Encoding UTF8 -Width 300
                }
...
# FUNCAO CORRIGIDA: Analise de Atualizacoes Completa
function Get-UpdatesAnalysisComplete {
    ...
    # HotFixes/Updates - SCRIPTBLOCKS CORRIGIDOS
    ...
    if ($hotfixResults.BestResult) {
        ...
        if ($securityUpdates.Count -gt 0) {
            ...
            $securityUpdates | Format-Table -AutoSize | Out-File -FilePath (Join-Path $updatesPath "${Timestamp}_Atualizacoes_Seguranca.txt") -Encoding UTF8 -Width 300
        }
        ...
        if ($recentUpdates.Count -gt 0) {
            ...
            $recentUpdates | Format-Table -AutoSize | Out-File -FilePath (Join-Path $updatesPath "${Timestamp}_Atualizacoes_Recentes.txt") -Encoding UTF8 -Width 300
        }
...
# Funcao Get-RemoteProgram (mantida do original) - CORRIGIDA para rede
    ...
    process {
        foreach ($Computer in $ComputerName) {
            ...
            if ($isLocal) {
                ...
                # IMPLEMENTACAO DE REDE CORRIGIDA
                ...
                try {
                    ...
                    if ($socket.ConnectAsync($Computer, 445).Wait($TimeoutMs)) {
                        Write-Verbose "Conectividade SMB confirmada para $Computer"
                        $socket.Close()
                        
                        # Tentar Registry remoto
                        try {
                        ...
                        Write-Warning "Falha na conectividade com $Computer"
                        $socket.Close()
                    }
                }
                catch {
                    Write-Warning "Erro na conectividade com $Computer : $($_.Exception.Message)"
                }
            }
...
function Get-SystemInformationComplete {
    ...
    try {
        ...
        if ($Results.OSInfo) {
            ...
            $Results.OSInfo | Export-Csv -Path (Join-Path $osPath "${Timestamp}_Info_Sistema.csv") -NoTypeInformation -Encoding UTF8
            $Results.OSInfo | Format-Table -AutoSize | Out-File -FilePath (Join-Path $osPath "${Timestamp}_Info_Sistema.txt") -Encoding UTF8 -Width 300
        }
...
function Get-NetworkAnalysisComplete {
    ...
    # GARANTIR CSV E TXT para todos os resultados
    ...
    if ($networkResults.Teams) {
        ...
        $networkResults.Teams | Format-Table -AutoSize | Out-File -FilePath (Join-Path $netPath "${Timestamp}_Teams_Rede.txt") -Encoding UTF8
        $networkResults.TeamMembers | Format-Table -AutoSize | Out-File -FilePath (Join-Path $netPath "${Timestamp}_Team_Members.txt") -Encoding UTF8
    }
...
# FUNCAO MELHORADA: Analise de disco com mais feedback e controle
function Get-DiskUsageAnalysisComplete {
    ...
    Write-Host "  AVISO: Esta operacao pode demorar varios minutos dependendo do tamanho dos drives" -ForegroundColor Yellow
    ...
    try {
        # Coletar informacoes dos drives primeiro
        Write-Host "  • Coletando informacoes dos drives..." -ForegroundColor White
        if ($isLocal) {
            try {
                ...
                Write-Host "    - CIM: $($drives.Count) drives encontrados" -ForegroundColor Green
            }
            catch {
                ...
                Write-Host "    - WMI: $($drives.Count) drives encontrados" -ForegroundColor Green
            }
        }
        else {
            try {
                ...
                $drives | Add-Member -NotePropertyName "Method" -NotePropertyValue "CIM"
                Write-Host "    - CIM Remoto: $($drives.Count) drives encontrados" -ForegroundColor Green
            }
            catch {
                $drives = Get-WmiObject -Class Win32_LogicalDisk -ComputerName $Computer -ErrorAction Stop | Where-Object { $_.DriveType -eq 3 }
                $drives | Add-Member -NotePropertyName "Method" -NotePropertyValue "WMI"
                Write-Host "    - WMI Remoto: $($drives.Count) drives encontrados" -ForegroundColor Green
            }
        }
        
        # Analisar cada drive
        foreach ($drive in $drives) {
            Write-Host "  • Analisando drive $($drive.DeviceID)..." -ForegroundColor White
            ...
            Write-Host "    - Drive $($drive.DeviceID): $usedPercent% usado ($usedGB GB de $totalGB GB)" -ForegroundColor $(if ($usedPercent -gt 90) { "Red" } elseif ($usedPercent -gt 80) { "Yellow" } else { "Green" })
            
            # Analisar pastas apenas para execucao local (evita lentidao em rede)
            if ($isLocal) {
                try {
                    $drivePath = $drive.DeviceID + "\"
                    Write-Host "    • Analisando estrutura de pastas do drive $($drive.DeviceID)..." -ForegroundColor Gray
                    Write-Host "      AVISO: Analise de pastas pode ser demorada..." -ForegroundColor Yellow
                    
                    $level1Folders = Get-ChildItem -Path $drivePath -Directory -ErrorAction SilentlyContinue
                    $folderCount = 0
                    $totalFolders = $level1Folders.Count
                    
                    foreach ($folder1 in $level1Folders) {
                        $folderCount++
                        Write-Host "      - Processando pasta $folderCount/$totalFolders : $($folder1.Name)" -ForegroundColor Gray
                        
                        try {
                            $folder1Size = 0
                            $folder1FileCount = 0
                            
                            # Nivel 1 - Arquivos diretos
                            ...
                            # Nivel 2 - Subpastas (limitado para evitar lentidao extrema)
                            $level2Folders = Get-ChildItem -Path $folder1.FullName -Directory -ErrorAction SilentlyContinue | Select-Object -First 20
                            foreach ($folder2 in $level2Folders) {
                                try {
                                    $level2Files = Get-ChildItem -Path $folder2.FullName -File -ErrorAction SilentlyContinue
                                    if ($level2Files) {
                                        $folder1Size += ($level2Files | Measure-Object -Property Length -Sum).Sum
                                        $folder1FileCount += $level2Files.Count
                                    }
                                    
                                    # Nivel 3 - Limitado ainda mais para evitar lentidao
                                    $level3Folders = Get-ChildItem -Path $folder2.FullName -Directory -ErrorAction SilentlyContinue | Select-Object -First 10
                                    foreach ($folder3 in $level3Folders) {
                                        try {
                                            $level3Files = Get-ChildItem -Path $folder3.FullName -File -ErrorAction SilentlyContinue
                                            if ($level3Files) {
                                                $folder3Size = ($level3Files | Measure-Object -Property Length -Sum).Sum
                                                $folder1Size += $folder3Size
                                                $folder1FileCount += $level3Files.Count
                                            }
                                        }
                                        catch { 
                                            Write-Host "        - Erro ao acessar: $($folder3.FullName)" -ForegroundColor DarkRed
                                        }
                                    }
                                }
                                catch { 
                                    Write-Host "        - Erro ao acessar: $($folder2.FullName)" -ForegroundColor DarkRed
                                }
                            }
                            ...
                            if ($folder1Size -gt 0) {
                                ...
                                Write-Host "        $($folder1.Name): $folderSizeGB GB ($folderPercent%)" -ForegroundColor DarkGray
                            }
                        }
                        catch { 
                            Write-Host "        - Erro ao processar: $($folder1.FullName)" -ForegroundColor DarkRed
                        }
                    }
                    
                    Write-Host "    • Analise de pastas concluida para drive $($drive.DeviceID)" -ForegroundColor Green
                }
                catch { 
                    Write-Warning "Erro na analise de pastas do drive $($drive.DeviceID): $($_.Exception.Message)"
                }
                
                # Agrupamento por tipo de uso
                try {
                    Write-Host "    • Criando agrupamento por uso do sistema..." -ForegroundColor Gray
                    ...
                    Write-Warning "Erro no agrupamento por uso do drive $($drive.DeviceID): $($_.Exception.Message)"
                }
            }
            else {
                Write-Host "    • Analise de pastas ignorada para execucao remota (otimizacao de performance)" -ForegroundColor Yellow
            }
        }
        
        # Salvar resultados + GARANTIR CSV E TXT
        Write-Host "  • Salvando resultados da analise de disco..." -ForegroundColor White
...
"@
        ...
        Write-Host "  Analise de disco detalhada concluida com sucesso!" -ForegroundColor Green
        Write-Host "    - $($diskAnalysis.Count) drives analisados" -ForegroundColor Green
        Write-Host "    - $($folderAnalysis.Count) diretorios catalogados" -ForegroundColor Green
        Write-Host "    - $($userAnalysis.Count) agrupamentos criados" -ForegroundColor Green
...
function Get-JavaProcessAnalysis {
    ...
    if ($suspiciousJava.Count -gt 0) {
        ...
        $suspiciousJava | Format-Table -AutoSize | Out-File -FilePath (Join-Path $procPath "${Timestamp}_Java_Suspeito.txt") -Encoding UTF8 -Width 300
    }
...
# FUNCAO CORRIGIDA: Analise de logs de eventos - Evitar variavel 'event'
    ...
    try {
        ...
        foreach ($logName in $logCategories) {
            ...
            try {
                ...
                if ($isLocal) {
                    $eventItems = Get-WinEvent -LogName $logName -MaxEvents 1000 -ErrorAction SilentlyContinue | 
                    ...
                    $eventItems = Get-WinEvent -ComputerName $Computer -LogName $logName -MaxEvents 1000 -ErrorAction SilentlyContinue | 
                        Where-Object { $_.LevelDisplayName -eq "Error" -or $_.LevelDisplayName -eq "Warning" -or $_.LevelDisplayName -eq "Critical" -or $_.Id -in $Script:EVENT_TRANSLATION.Keys }
                }
                
                foreach ($eventItem in $eventItems) {
                    $translatedDescription = if ($Script:EVENT_TRANSLATION.ContainsKey($eventItem.Id.ToString())) {
                        $Script:EVENT_TRANSLATION[$eventItem.Id.ToString()]
                    }
                    else {
                        $eventItem.LevelDisplayName + " - " + $eventItem.TaskDisplayName
                    }
                    ...
                    $eventAnalysis += [PSCustomObject]@{
                        ...
                        TaskCategory          = $eventItem.TaskDisplayName
                        Description           = $eventItem.Message
                        TranslatedDescription = $translatedDescription
                        Computer              = $Computer
                        UserId                = $eventItem.UserId
                        ProcessId             = $eventItem.ProcessId
                        ThreadId              = $eventItem.ThreadId
                        Severity              = switch ($eventItem.LevelDisplayName) {
                            "Critical" { "CRITICO" }
                            "Error" { "ERRO" }
                            "Warning" { "AVISO" }
                            default { "INFO" }
                ...
                Write-Host "    - Coletados $(($eventItems | Measure-Object).Count) eventos de $logName" -ForegroundColor Gray
            }
            catch {
...
"@
        ...
        # GARANTIR CSV E TXT para cada categoria
        foreach ($logName in $logCategories) {
...
"@
    ...
    foreach ($eventItem in $sortedEvents) {
        $timelineReport += "[$($eventItem.TimeCreated.ToString('dd/MM/yyyy HH:mm:ss'))] $($eventItem.Severity) - ID:$($eventItem.Id)`n"
        $timelineReport += "Fonte: $($eventItem.Source) | Categoria: $($eventItem.LogName)`n"
        $timelineReport += "Descricao: $($eventItem.TranslatedDescription)`n`n"
    }
        ...
        if ($categoryEvents.Count -gt 0) {
            ...
            foreach ($criticality in $criticityOrder) {
                ...
                if ($criticalEvents.Count -gt 0) {
                    ...
                    foreach ($eventItem in $criticalEvents) {
                        $familyReport += "[$($eventItem.TimeCreated.ToString('dd/MM/yyyy HH:mm:ss'))] ID:$($eventItem.Id) - $($eventItem.TranslatedDescription)`n"
                    }
                    $familyReport += "`n"
                }
            }
...
# Funcao para criar pagina HTML CORRIGIDA
            ...
            if (fileExtension === 'csv') {
                fetch(filePath)
                    ...
                    .then(data => {
                        ...
                        let html = '<div style="padding: 20px;"><h3>' + fileName + '</h3>';
...
"@
    ...
    <div style="text-align: center; margin-top: 20px; color: #7f8c8d; font-size: 12px;">
        <p>Baseline NMR5 v10.2.4 - Sistema de Auditoria Tecnica SCADA | Gerado automaticamente</p>
    </div>
</body>
</html>
"@
...
# NOVA FUNCAO: Gerar relatorio preliminar (sem disco)
function New-PreliminaryReport {
    param(
        [string]$Computer,
        [string]$OutputPath,
        [string]$Domain,
    ...
    Write-Host "Gerando relatorio preliminar (sem analise de disco)..." -ForegroundColor Magenta
    ...
    try {
        ...
        $preliminaryReport = @"
################################################################################
              RELATORIO PRELIMINAR DE AUDITORIA TECNICA - Sistema SCADA
################################################################################
...
$Script:SCRIPT_HEADER
Duracao da Analise (sem disco): $auditDuration

*** RELATORIO PRELIMINAR - ANALISE DE DISCO SERA ADICIONADA AO FINAL ***
...
$(if ($HardwareInfo.Memory) { "Memoria RAM: $([math]::Round(($HardwareInfo.Memory | Measure-Object Capacity -Sum).Sum/1GB, 2)) GB" } else { "Memoria RAM: Nao disponivel" })
...
"@
        ...
        if ($NetworkResults.Adapters) {
            $preliminaryReport += "• Adaptadores de Rede: $($NetworkResults.Adapters.Count)`n"
        }
        
        if ($NetworkResults.Teams -and $NetworkResults.Teams.Count -gt 0) {
            $preliminaryReport += "• Teams Configurados: $($NetworkResults.Teams.Count)`n"
            foreach ($team in $NetworkResults.Teams) {
                $preliminaryReport += "  - $($team.Name): $($team.Status) - $($team.TeamingMode)`n"
            }
        }
        else {
            $preliminaryReport += "• Teams de Rede: Nenhum configurado`n"
        }
        
        if ($NetworkResults.Connections) {
            $preliminaryReport += "• Conexoes TCP Ativas: $($NetworkResults.Connections.Count)`n"
        }
...
"@
        ...
        if ($suspiciousJava -gt 0) {
            $preliminaryReport += "ALERTA: $suspiciousJava processos Java suspeitos detectados - Requerem verificacao manual`n"
        }
        
        if ($suspiciousNetwork -gt 0) {
            $preliminaryReport += "ALERTA: $suspiciousNetwork atividades de rede incomuns detectadas`n"
        }
        
        if ($suspiciousSoftware.Count -gt 0) {
            $preliminaryReport += "ALERTA: $($suspiciousSoftware.Count) programas suspeitos detectados:`n"
            foreach ($software in ($suspiciousSoftware | Select-Object -First 5)) {
                $preliminaryReport += "  - $($software.ProgramName)`n"
            }
            if ($suspiciousSoftware.Count -gt 5) {
                $preliminaryReport += "  ... e mais $($suspiciousSoftware.Count - 5) programas`n"
            }
        }
...
"@
        ...
        if ($criticalEvents -gt 0) {
            ...
            foreach ($criticalEvent in $recentCritical) {
                $preliminaryReport += "$($criticalEvent.TimeCreated.ToString('dd/MM/yyyy HH:mm')) - ID:$($criticalEvent.Id) - $($criticalEvent.TranslatedDescription)`n"
            }
        }
        else {
            $preliminaryReport += "Nenhum evento critico nos ultimos 7 dias.`n"
        }

        $preliminaryReport += @"

 Recomendacoes Tecnicas Preliminares:
================================================================================
"@
        ...
        if ($recommendations.Count -eq 0) {
            $preliminaryReport += "• Sistema esta funcionando dentro dos parametros normais (analise preliminar)`n"
        }
        else {
            $preliminaryReport += ($recommendations -join "`n") + "`n"
        }
...
################################################################################
           *** ANALISE DE DISCO SERA ADICIONADA AO RELATORIO FINAL ***
################################################################################
"@

        # Salvar relatorio preliminar + GARANTIR CSV E TXT
        $preliminaryReport | Out-File -FilePath (Join-Path $reportPath "Relatorio_Preliminar_$Computer.txt") -Encoding UTF8
        ...
        $jsonReport | ConvertTo-Json -Depth 3 | Out-File -FilePath (Join-Path $reportPath "Relatorio_Preliminar_$Computer.json") -Encoding UTF8
        
        Write-Host "Relatorio preliminar gerado com sucesso!" -ForegroundColor Green
        Write-Host "  - Arquivo TXT: $(Join-Path $reportPath "Relatorio_Preliminar_$Computer.txt")" -ForegroundColor Gray
        Write-Host "  - Arquivo JSON: $(Join-Path $reportPath "Relatorio_Preliminar_$Computer.json")" -ForegroundColor Gray
        
        return @{
            ReportPath      = (Join-Path $reportPath "Relatorio_Preliminar_$Computer.txt")
            JsonPath        = (Join-Path $reportPath "Relatorio_Preliminar_$Computer.json")
            Status          = $jsonReport.Status
            Recommendations = $recommendations
        }
    }
    catch {
        Write-Warning "Erro ao gerar relatorio preliminar: $($_.Exception.Message)"
        return $null
    }
}
...
function New-ConsolidatedReportComplete {
    ...
    Write-Host "Atualizando relatorio com analise completa de disco..." -ForegroundColor Cyan
    ...
    try {
        ...
        $consolidatedReport = @"
################################################################################
                    RELATORIO DE AUDITORIA TECNICA COMPLETO - Sistema SCADA
################################################################################
...
Data/Hora da Auditoria: $(Get-Date -Format 'dd/MM/yyyy HH:mm:ss')
$Script:SCRIPT_HEADER
Duracao da Analise Completa: $auditDuration
...
"@
        ...
        if ($criticalEvents -gt 0) {
            ...
            foreach ($criticalEvent in $recentCritical) {
                $consolidatedReport += "$($criticalEvent.TimeCreated.ToString('dd/MM/yyyy HH:mm')) - ID:$($criticalEvent.Id) - $($criticalEvent.TranslatedDescription)`n"
            }
        }
        else {
...
################################################################################
                          FIM DO RELATORIO DE AUDITORIA COMPLETO
################################################################################
"@

        # Salvar relatorio consolidado + GARANTIR CSV E TXT
        ...
        $jsonReport = @{
            ...
            Metrics         = @{
                ...
                SuspiciousJava     = $suspiciousJava
                SuspiciousNetwork  = $suspiciousNetwork
                DiskDrives         = if ($DiskResults.DiskAnalysis) { $DiskResults.DiskAnalysis.Count } else { 0 }
                CriticalDiskSpace  = if ($DiskResults.DiskAnalysis) { ($DiskResults.DiskAnalysis | Where-Object { $_.UsedPercent -gt 90 }).Count } else { 0 }
            }
            Recommendations = $recommendations
            Status          = if ($recommendations.Count -eq 0) { "OK" } elseif ($recommendations -match "URGENTE") { "CRITICO" } else { "ATENCAO" }
        }
        ...
        Write-Host "Relatorio consolidado COMPLETO gerado com sucesso!" -ForegroundColor Green
...
function Start-SystemAudit {
    ...
    Write-Host "                    BASELINE NMR5 10.2.4 - PIC.EE.0246 FINAL" -ForegroundColor Cyan
    Write-Host "################################################################################" -ForegroundColor Cyan
    ...
    # Iniciar coleta de dados (SEM DISCO)
    Write-Host "`n################################################################################" -ForegroundColor Green
    Write-Host "                        INICIANDO COLETA DE DADOS (FASE 1/2)" -ForegroundColor Green
    Write-Host "################################################################################" -ForegroundColor Green
    
    # 1. Informacoes do Sistema
    Write-Host "`n[1/10] Coletando informacoes completas do sistema..." -ForegroundColor Yellow
    $systemInfo = Get-SystemInformationComplete -Computer $Computer -OutputPath $OutputBasePath -Timestamp $timestamp
    
    # 2. Hardware 
    Write-Host "`n[2/10] Coletando informacoes de hardware..." -ForegroundColor Yellow
    $hardwareInfo = Get-HardwareInformationComplete -Computer $Computer -OutputPath $OutputBasePath -Timestamp $timestamp
    
    # 3. BIOS
    Write-Host "`n[3/10] Coletando informacoes de BIOS..." -ForegroundColor Yellow
    $biosInfo = Get-BIOSInformationComplete -Computer $Computer -OutputPath $OutputBasePath -Timestamp $timestamp
    
    # 4. Software instalado
    Write-Host "`n[4/10] Analisando software instalado..." -ForegroundColor Yellow
    ...
    # GERAR RELATORIO PRELIMINAR
    Write-Host "`n################################################################################" -ForegroundColor Magenta
    Write-Host "                           GERANDO RELATORIO PRELIMINAR" -ForegroundColor Magenta
    Write-Host "################################################################################" -ForegroundColor Magenta
    
    $preliminaryReport = New-PreliminaryReport -Computer $Computer -OutputPath $OutputBasePath -Domain $Domain -Timestamp $timestamp -SystemInfo $systemInfo -SoftwareList $softwareList -HardwareInfo $hardwareInfo -BIOSInfo $biosInfo -ServicesInfo $servicesInfo -ProcessInfo $processInfo -DriversInfo $driversInfo -UpdatesInfo $updatesInfo -NetworkResults $networkResults -JavaResults $javaResults -EventAnalysis $eventAnalysis -SecurityResults $securityResults
    
    # FASE 2: ANALISE DE DISCO (ULTIMA ETAPA)
    Write-Host "`n################################################################################" -ForegroundColor Red
    Write-Host "                        INICIANDO ANALISE DE DISCO (FASE 2/2)" -ForegroundColor Red
    Write-Host "################################################################################" -ForegroundColor Red
    Write-Host "IMPORTANTE: Esta etapa pode ser muito demorada. O relatorio preliminar ja esta disponivel." -ForegroundColor Yellow
    Write-Host "Arquivo preliminar: $(if ($preliminaryReport) { $preliminaryReport.ReportPath } else { 'Nao disponivel' })" -ForegroundColor Yellow
    Write-Host "`nIniciando analise detalhada de disco..." -ForegroundColor Red
    
    $diskResults = Get-DiskUsageAnalysisComplete -Computer $Computer -OutputPath $OutputBasePath -Timestamp $timestamp
    
    # Gerar relatorio final COMPLETO
    Write-Host "`n################################################################################" -ForegroundColor Green
    Write-Host "                           GERANDO RELATORIO FINAL COMPLETO" -ForegroundColor Green
    Write-Host "################################################################################" -ForegroundColor Green
    ...
    if ($diskResults.DiskAnalysis -and $diskResults.DiskAnalysis.Count -gt 0) {
        Write-Host "`nAnalise de disco concluida:" -ForegroundColor Cyan
        foreach ($disk in $diskResults.DiskAnalysis) {
            Write-Host "  • Drive $($disk.Drive): $($disk.UsedPercent)% usado - Status: $($disk.Status)" -ForegroundColor $(if ($disk.Status -eq "CRITICO") { "Red" } elseif ($disk.Status -eq "ATENCAO") { "Yellow" } else { "Green" })
        }
    }
...
function Invoke-SystemAudit {
    ...
    try {
        ...
        # Resumo de performance de disco
        Write-Host "`nResumo da analise de disco:" -ForegroundColor Cyan
        foreach ($result in $results) {
            if ($result.DiskResults -and $result.DiskResults.DiskAnalysis) {
                $computerName = $result.OutputPath -replace ".*[\\/]([^\\/]+)[\\/]*$", '$1'
                $criticalDisks = $result.DiskResults.DiskAnalysis | Where-Object { $_.UsedPercent -gt 90 }
                $warningDisks = $result.DiskResults.DiskAnalysis | Where-Object { $_.UsedPercent -gt 80 -and $_.UsedPercent -le 90 }
                
                if ($criticalDisks.Count -gt 0) {
                    Write-Host "  CRITICO - $computerName : $($criticalDisks.Count) drives com >90% uso" -ForegroundColor Red
                }
                elseif ($warningDisks.Count -gt 0) {
                    Write-Host "  ATENCAO - $computerName : $($warningDisks.Count) drives com >80% uso" -ForegroundColor Yellow
                }
                else {
                    Write-Host "  OK - $computerName : Todos os drives com utilizacao normal" -ForegroundColor Green
                }
            }
        }
