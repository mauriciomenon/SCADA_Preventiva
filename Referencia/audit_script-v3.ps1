    ...
    try {
        ...
        foreach ($drive in $drives) {
            ...
            $diskAnalysis += [PSCustomObject]@{
                ...
                Status = if ($usedPercent -gt 90) { "CRITICO" } 
                         elseif ($usedPercent -gt 80) { "ATENCAO" } 
                         else { "OK" }
                Method = $drive.Method
            }
                ...
                try {
                    ...
                    if ($suspiciousIndicators.Count -gt 0) {
                        $suspiciousJava += [PSCustomObject]@{
                            ...
                            RiskLevel = if ($suspiciousIndicators.Count -gt 2) { "ALTO" } 
                                       elseif ($suspiciousIndicators.Count -gt 1) { "MEDIO" } 
                                       else { "BAIXO" }
                        }
                    }
                }
...
function New-ConsolidatedReportComplete {
    ...
    try {
        ...
        $totalServices = if ($SystemInfo.Services) { $SystemInfo.Services.Count } else { 0 }
        $runningServices = if ($SystemInfo.Services) { ($SystemInfo.Services | Where-Object { $_.State -eq 'Running' }).Count } else { 0 }
        $totalProcesses = if ($SystemInfo.Processes) { $SystemInfo.Processes.Count } else { 0 }
        $totalDrivers = if ($SystemInfo.SystemDrivers) { $SystemInfo.SystemDrivers.Count } else { 0 }
        $loadedDrivers = if ($SystemInfo.SystemDrivers) { ($SystemInfo.SystemDrivers | Where-Object { $_.State -eq 'Running' }).Count } else { 0 }
        $totalUpdates = if ($SystemInfo.HotFixes) { $SystemInfo.HotFixes.Count } else { 0 }
        ...
        $diskCritical = if ($DiskResults.DiskAnalysis) { ($DiskResults.DiskAnalysis | Where-Object { $_.UsedPercent -gt 90 }).Count } else { 0 }
        $diskWarning = if ($DiskResults.DiskAnalysis) { ($DiskResults.DiskAnalysis | Where-Object { $_.UsedPercent -gt 80 -and $_.UsedPercent -le 90 }).Count } else { 0 }
...
"@
        ...
        if ($NetworkResults.Connections) {
            $establishedCount = if ($NetworkResults.Connections.RemoteAddress) { 
                ($NetworkResults.Connections | Where-Object { $_.State -eq "Established" }).Count 
            } else { 
                ($NetworkResults.Connections | Where-Object { $_ -match "ESTABLISHED" }).Count 
            }
            $consolidatedReport += "