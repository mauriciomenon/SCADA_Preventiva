...
function Invoke-AllMethodsWithQuality {
    ...
    try {
        ...
        if ($results["CIM"]) {
            ...
            if (-not (Test-Path $cimPath)) {
                New-Item -ItemType Directory -Path $cimPath -Force | Out-Null 
            }
    ...
    try {
        ...
        if ($results["WMIC"]) {
            ...
            if (-not (Test-Path $wmicPath)) {
                New-Item -ItemType Directory -Path $wmicPath -Force | Out-Null 
            }
...
function Get-HardwareInformationComplete {
    ...
    $wmicCPU = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp cpu get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        }
        else {
            cmd /c "wmic cpu get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        } }
    ...
    $wmicRAM = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp memorychip get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Capacity } 
        }
        else {
            cmd /c "wmic memorychip get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Capacity } 
        } }
    ...
    $wmicMB = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp computersystem get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        }
        else {
            cmd /c "wmic computersystem get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        } }
...
function Get-BIOSInformationComplete {
    ...
    $wmicBIOS = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp bios get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Version } 
        }
        else {
            cmd /c "wmic bios get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Version } 
        } }
    ...
    $wmicBoard = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp baseboard get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Product } 
        }
        else {
            cmd /c "wmic baseboard get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Product } 
        } }
...
function Get-ServicesAnalysisComplete {
    ...
    $wmicSvc = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp service get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        }
        else {
            cmd /c "wmic service get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        } }
...
function Get-ProcessAnalysisComplete {
    ...
    $wmicProc = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp process get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        }
        else {
            cmd /c "wmic process get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        } }
    ...
    return @{
        ...
        SuspiciousProcesses = if ($suspiciousProcesses) {
            $suspiciousProcesses 
        }
        else {
            @() 
...
function Get-DriversAnalysisComplete {
    ...
    $wmicSysDriver = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp sysdriver get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        }
        else {
            cmd /c "wmic sysdriver get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.Name } 
        } }
...
"@
    ...
    return @{
        ...
        PnPDrivers      = if ($pnpDrivers) {
            $pnpDrivers 
        }
        else {
            @() 
        }
        UnsignedDrivers = if ($unsignedDrivers) {
            $unsignedDrivers 
        }
        else {
            @() 
...
function Get-UpdatesAnalysisComplete {
    ...
    $wmicHotfix = { param($comp) if ($comp -and $comp -ne "localhost") {
            cmd /c "wmic /node:$comp qfe get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.HotFixID } 
        }
        else {
            cmd /c "wmic qfe get /format:csv 2>nul" | ConvertFrom-Csv | Where-Object { $_.HotFixID } 
        } }
...
"@
    ...
    return @{
        ...
        SecurityUpdates = if ($securityUpdates) {
            $securityUpdates 
        }
        else {
            @() 
        }
        RecentUpdates   = if ($recentUpdates) {
            $recentUpdates 
        }
        else {
            @() 
...
function New-ConsolidatedReportComplete {
    ...
    try {
        ...
        $totalServices = if ($ServicesInfo.Services) {
            $ServicesInfo.Services.Count 
        }
        else {
            0 
        }
        $runningServices = if ($ServicesInfo.RunningServices) {
            $ServicesInfo.RunningServices.Count 
        }
        else {
            0 
        }
        $totalProcesses = if ($ProcessInfo.Processes) {
            $ProcessInfo.Processes.Count 
        }
        else {
            0 
        }
        $totalDrivers = if ($DriversInfo.SystemDrivers) {
            $DriversInfo.SystemDrivers.Count 
        }
        else {
            0 
        }
        $loadedDrivers = if ($DriversInfo.SystemDrivers) {
            ($DriversInfo.SystemDrivers | Where-Object { $_.State -eq 'Running' }).Count 
        }
        else {
            0 
        }
        $totalUpdates = if ($UpdatesInfo.HotFixes) {
            $UpdatesInfo.HotFixes.Count 
        }
        else {
            0 
        }
